...
export function urlSearchParamsGet(uriQueryParameters: Uri["queryParameters"], name: string): ReturnType<URLSearchParams["get"]> {
    const matched = uriQueryParameters.find((param) => (param.name === name));
...
export class Uri {
    ...
    public get absoluteUri(): string {
        ...
        if (this.path) {
            ...
            if (this.noSchemeSeparator) {
                ...
                encodedPath = parts.map(p => encodeURI(decodeURI(p.replace(/%/g,"%25")))).join("/").replace(/%25/g,"%");
            }
     ...
     */
    public noSchemeSeparator: boolean | undefined;
     ...
     */
    public hashString: string | undefined;
    ...
    public getEffectivePort(): number {
        if (this.port) {
            ...
            if (StringUtil.equals(this.scheme, "http", true)) {
                return 80;
            } else if (StringUtil.equals(this.scheme, "https", true)) {
                return 443;
            } else {
                return 0;
            }
    ...
    public getQueryParam(name: string): string | null | undefined {
        ...
        if (this.queryParameters) {
            const matchingPairs = this.queryParameters.filter(p => StringUtil.equals(p.name, name, true));
    ...
    public removeQueryParam(name: string): void {
        if (this.queryParameters) {
            this.queryParameters = this.queryParameters.filter(p => !StringUtil.equals(p.name, name, true));
        }
    }
    ...
    private _setFromUriString(uriString: string, options?: IUriParseOptions) {
        ...
        const hashSplit = singleSplit(uri, "#");
        if (hashSplit) {
            uri = hashSplit.part1;
            this.hashString = decodeUri(hashSplit.part2);
        } else {
            this.hashString = "";
        }

        // Parse the query parameters
        const querySplit = singleSplit(uri, "?");
        if (querySplit) {
        ...
        const schemeSplit = singleSplit(uri, ":");
        if (schemeSplit) {
            ...
            if (uri.substr(0, 2) === "//") {
                ...
                const pathSplit = singleSplit(uri, "/");
                if (pathSplit) {
                    uri = pathSplit.part1;
                ...
                const portSplit = singleSplit(uri, ":");
                if (portSplit) {
...
function singleSplit(value: string, separator: string): { part1: string; part2: string } | undefined {
 ...
 */
export function parseQueryString(paramString: string): IQueryParameter[] {
    const params: IQueryParameter[] = [];
    paramString.split("&").forEach(pair => {
        if (pair) {
            const valueSplit = singleSplit(pair, "=");
            if (valueSplit) {
                params.push({
                    name: decodeUri(valueSplit.part1),
                    value: decodeUri(valueSplit.part2)
                });
            } else {
                params.push({
                    name: decodeUri(pair),
 ...
 */
export function toQueryString(params: IQueryParameter[]): string {
...
/**
 * Resolve entries of . and .. in the path
 * @example resolveRelativePath("a/b/../c/./d") === "a/c/d"
 */
export function resolveRelativePath(path: string) {
    const segments = path.split("/").filter(s => s !== ".");

    let index: number;
    while ((index = segments.indexOf("..")) > 0) {
        // Drop the ".." entry and the previous one
    ...
    for (let charIndex = 0, routeTemplateLen = routeTemplate.length; charIndex < routeTemplateLen; charIndex++) {
        ...
        if (paramStartIndex >= 0) {
            ...
            if (c === KeyCodes.startCurlyBrace && routeTemplate.charCodeAt(charIndex + 1) !== KeyCodes.startCurlyBrace) {
                ...
                if (
                    (c === KeyCodes.startCurlyBrace && routeTemplate.charCodeAt(charIndex + 1) === KeyCodes.startCurlyBrace) ||
...
export function getBestRouteMatch(routeCollection: IParsedRoute[], routeValues: { [name: string]: string }): IRouteMatchResult | undefined {
    ...
    for (const route of routeCollection) {
        ...
        // We have a better match if
        // - More route segments were matched than the previous match
        // - OR the same number of segments were matched, but the new match matched all segments and the prior match did not.
        if (
            match &&
            (!bestMatch ||
                match.matchedParametersCount > bestMatch.matchedParametersCount ||
                (match.matchedParametersCount === bestMatch.matchedParametersCount &&
                    match.unmatchedParametersCount === 0 &&
                    bestMatch.unmatchedParametersCount > 0))
        ) {
            bestMatch = match;
...
export interface IRouteMatchResult {
    ...
    /**
     * The number of parameters that were not replaced
     */
    unmatchedParametersCount: number;
}
 ...
 */
export function replaceParsedRouteValues(
    parsedRoute: IParsedRoute,
    routeValues: { [name: string]: string | number | undefined },
    continueOnUnmatchedSegements?: boolean
    ...
    const unmatchedParametersCount = parsedRoute.segments.filter(s => s.paramName).length - matchedParametersCount;

    return {
        url: urlParts.join(""),
        matchedParameters,
        matchedParametersCount,
        unmatchedParametersCount
    };
}
...
/**
 * Check if the given urls have the same origin (scheme, host and port)
 *
...
export function isSameOrigin(url1: string, url2: string): boolean {
    ...
    return StringUtil.equals(uri1.host || '', uri2.host || '', true) && uri1.getEffectivePort() === uri2.getEffectivePort();
}
